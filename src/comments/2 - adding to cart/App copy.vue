...
// 1.0 Разработка корзины товаров. И первое что нужно сделать — это её отображать. Вёрстка уже готова и осталось подключить к ней функционал. Для начала нужно, чтобы по клике на закладку корзины появлялся компонент DrawerComp. Точнее он будет отображаться только в случае, если флажок drawerOpened в положении true, что будет означать, что корзина открыта. (Изначально она будет закрыта, т.е. в положении false)
...
// 1.3 Т.к. у нас есть стрелочка закрытия корзины внутри неё, то нам нужно прокинуть функцию закрытия корзины товаров в дочерний компонент DrawerHeader дочернего компонента DrawerComp, а это уже кейс для props drilling, и тут вполне логично будет использовать метод, использующий provide & inject. Создадим некий объединяющий обе функции открытия и закрытия корзины "cart". Теперь у нас будут две глобальные переменные с вышеозначенными функциями, которые можно использовать в любом дочернем компоненте App.
